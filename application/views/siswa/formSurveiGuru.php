<form id="signupForm" class="ml-2" action="<?php echo site_url('siswa/aksiGuru');?>" method="post" novalidate="novalidate">
<div class="card mb-3 border-info">
    ...
    <?php $i = 1; foreach ($detail as $detail) : ?>
        <h3>Nama Dosen : <?= $detail['nama']; ?></h3>
        <p class="text-danger float-right">Batas Isi Kuesioner : <?= $detail['tgl_selesai']; ?></p>
        <input type="hidden" name="id_survei" id="" value="<?= $detail['id_survei_guru']; ?>">
        <input type="hidden" name="guru" id="" value="<?= $detail['id_guru']; ?>">
    <?php endforeach; ?>
    <!-- end detail -->
    </div>
</div>   
